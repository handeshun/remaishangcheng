...
- (void)_init
{
    ...
    if(!self.pageLabel){
        self.pageLabel = [[UILabel alloc] initWithFrame:CGRectMake(self.frame.size.width - 50, self.frame.size.height - 50, 40, 40)];
    }
    self.pageLabel.text = [NSString stringWithFormat:@"%d/%d", 1, self.count];
    self.pageLabel.backgroundColor = [UIColor colorWithPatternImage:[UIImage imageNamed:@"image_page_bg"]];
    self.pageLabel.textColor = [UIColor whiteColor];
    self.pageLabel.font = [UIFont systemFontOfSize:16];
    self.pageLabel.textAlignment = NSTextAlignmentCenter;
    self.pageLabel.tag = 1;
...
//    NSInteger currentPage = self.pageControl.currentPage;
    NSInteger currentPage = self.pageLabel.tag;
    ...
    if(currentPage == self.count){
        UIImageView *imageView = (UIImageView *) [self.scrollView viewWithTag:(0 + kStartTag)];
        [self.scrollView scrollRectToVisible:imageView.frame animated:animated];

        self.pageLabel.tag = 1;
        self.pageLabel.text = [NSString stringWithFormat:@"%d/%d", 1, self.count];
    }else {
        UIImageView *imageView = (UIImageView *) [self.scrollView viewWithTag:(currentPage + kStartTag)];
        [self.scrollView scrollRectToVisible:imageView.frame animated:animated];

        self.pageLabel.tag = nextPage;
        self.pageLabel.text = [NSString stringWithFormat:@"%d/%d", nextPage, self.count];
    }
}
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    self.pageLabel.tag = currentIndex;
    if(currentIndex >= self.count){
        currentIndex = 0;
    }
    self.pageLabel.text = [NSString stringWithFormat:@"%d/%d", currentIndex+1, self.count];
}

- (void)setHidePageControl:(BOOL)hidePageControl
{
    self.pageLabel.hidden = hidePageControl;
}
...
//    self.pageControl.currentPage = 0;
    self.pageLabel.text = [NSString stringWithFormat:@"%d/%d", 1, count];
    self.pageLabel.tag = 1;
