...
//    [bezierPath stroke];
    ...
    if (self.selected) {
        ...
        [bezierPath setLineWidth:self.lineHeight];
...
@property (nonatomic) NSNumber *tabLocationFrameY;
...
@synthesize tabLocationFrameY = _tabLocationFrameY;
...
- (void)layoutSubviews {
    ...
    frame.origin.x = 0.0;
    if(self.tabLocation.doubleValue == 1.0){
        frame.origin.y = topLayoutGuide;
    }else if(self.tabLocation.doubleValue == 0.0){
        frame.origin.y = CGRectGetHeight(self.view.frame) - [self.tabHeight floatValue];
    }else if(self.tabLocation.doubleValue == 2.0){
        frame.origin.y = self.tabLocationFrameY.doubleValue;
    }
//    frame.origin.y = [self.tabLocation boolValue] ? topLayoutGuide : CGRectGetHeight(self.view.frame) - [self.tabHeight floatValue];
    ...
    if(self.tabLocation.doubleValue == 1.0){
        ...
        frame.origin.y = self.tabLocationFrameY.doubleValue + CGRectGetHeight(self.tabsView.frame);
    }
...
- (void)setTabLocationFrameY:(NSNumber *)tabLocationFrameY {
    _tabLocationFrameY = tabLocationFrameY;
}
...
- (NSNumber *)tabLocationFrameY {

    if (!_tabLocationFrameY) {
        CGFloat value = kTabLocationFrameY;
        if ([self.delegate respondsToSelector:@selector(viewPager:valueForOption:withDefault:)])
            value = [self.delegate viewPager:self valueForOption:ViewPagerOptionTabLocationFrameY withDefault:value];
        self.tabLocationFrameY = [NSNumber numberWithFloat:value];
    }
    return _tabLocationFrameY;
}
...
- (CGFloat)valueForOption:(ViewPagerOption)option {
    ...
    switch (option) {
        ...
        case ViewPagerOptionTabLocationFrameY:
            return [[self tabLocationFrameY] floatValue];
...
- (void)defaultSetup {
    ...
    if (!self.tabsView) {
        ...
        layer.frame = CGRectMake(0, self.tabsView.frame.size.height - lineHeight, kScreen_Width, lineHeight);
        layer.backgroundColor = [UIColor colorWithHexString:@"#cdcdcd"].CGColor;
        [self.tabsView.layer addSublayer:layer];
    ...
    for (NSUInteger i = 0; i < self.tabCount; i++) {
        ...
        if(self.tabLocationFrameY.doubleValue > 0.0) {
            tabView.lineHeight = 2.0f;
        }else{
            tabView.lineHeight = 5.0f;
        }
        
        CGRect frame = tabView.frame;
        if(self.tabLocationFrameY.doubleValue <= 0) {
            frame.origin.x = (kScreen_Width - 180) / 2 + contentSizeWidth;
        }else{
            frame.origin.x = contentSizeWidth;
        }
        frame.size.width = [self.tabWidth floatValue];
