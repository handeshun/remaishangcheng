...
-(BOOL) init:(Order *)_order withPayment:(Payment *)payment {
    NSDictionary *paramObject = [NSJSONSerialization JSONObjectWithData:[payment.config dataUsingEncoding:NSUTF8StringEncoding] options:kNilOptions error:nil];
...
-(NSString *)sendPrepay:(NSMutableDictionary *)prePayParams {
    ...
    if(prepayid != nil){
        ...
        [defaults setObject: prepayid forKey: order.sn];
...
- ( NSMutableDictionary *)pay {
    ...
    [packageParams setObject:[NSString stringWithFormat:@"%@", order.sn] forKey:@"body"];        //订单描述，展示给用户
    [packageParams setObject:[NSString stringWithFormat:@"%@/api/shop/b_wechatMobilePlugin_payment-callback/execute.do", kBaseUrl] forKey:@"notify_url"];  //支付结果异步通知
    [packageParams setObject: order.sn           forKey:@"out_trade_no"];//商户订单号
    [packageParams setObject: @"127.0.0.1"    forKey:@"spbill_create_ip"];//发器支付的机器ip
    [packageParams setObject: [NSString stringWithFormat:@"%.0f", order.needPayMoney * 100]       forKey:@"total_fee"];       //订单金额，单位为分
//    [packageParams setObject: @"1"       forKey:@"total_fee"];       //订单金额，单位为分
    ...
    NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
    prePayid = [defaults objectForKey:order.sn];
