...
+ (NSString *) generateOrderString:(Order *)order withPayment:(Payment *)payment{
    ...
    alipayOrder.tradeNO = order.sn; //订单ID（由商家自行制定）
    alipayOrder.productName = [NSString stringWithFormat:@"商城订单：%@", order.sn];
    alipayOrder.productDescription = [NSString stringWithFormat:@"订单：%@", order.sn]; //商品描述
    alipayOrder.amount = [NSString stringWithFormat:@"%.2f", order.needPayMoney]; //商品价格

//    alipayOrder.amount = @"0.01"; //商品价格

    alipayOrder.notifyURL = [NSString stringWithFormat:@"%@/api/shop/b_alipayMobilePlugin_payment-callback/execute.do", kBaseUrl]; //回调URL
