...
- (void)customInit {
    ...
    //提示信息
    ESLabel *tipLbl = [[ESLabel alloc] initWithText:@"对准条形码/二维码到框内即可扫描" textColor:[UIColor whiteColor] fontSize:12];
    [self addSubview:tipLbl];
    [tipLbl mas_makeConstraints:^(MASConstraintMaker *make) {
        make.centerX.equalTo(self);
        make.top.equalTo(@((kScreen_Height - (kScreen_Width - 100)) / 2 - 20));
    }];

    //闪光灯按钮
    UIButton *lightBtn = [UIButton new];
    [lightBtn setBackgroundImage:[UIImage imageNamed:@"barcode_button_normal.png"] forState:UIControlStateNormal];
    [lightBtn setBackgroundImage:[UIImage imageNamed:@"barcode_button_press.png"] forState:UIControlStateSelected];
    [lightBtn setImage:[UIImage imageNamed:@"barcode_light_off.png"] forState:UIControlStateNormal];
    [lightBtn setImage:[UIImage imageNamed:@"barcode_light_on.png"] forState:UIControlStateSelected];
    ...
    [lightBtn mas_makeConstraints:^(MASConstraintMaker *make) {
        ...
        make.centerX.equalTo(self);
        make.top.equalTo(tipLbl.mas_bottom).offset(kScreen_Width - 100 + 20);
    }];
}
...
- (void)startMoveLine {
    ...
    self.qrLine = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"ff_QRCodeScanLine.png"]];
