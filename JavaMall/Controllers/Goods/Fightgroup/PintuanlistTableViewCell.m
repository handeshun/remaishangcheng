...
@interface PintuanlistTableViewCell()<UIAlertViewDelegate>
...
@property(nonatomic,strong)UILabel *dipositLab;
...
@implementation PintuanlistTableViewCell
...
-(instancetype)initWithStyle:(UITableViewCellStyle)style reuseIdentifier:(NSString *)reuseIdentifier{
    ...
    if (self=[super initWithStyle:style reuseIdentifier:reuseIdentifier]) {
        ...
        dayLab.adjustsFontSizeToFitWidth = YES;
...
//        self.numBuyLab=numBuyLab;
        ...
        countLab.font = LBFont(28);
        countLab.textAlignment = NSTextAlignmentLeft;
        countLab.textColor = LBColor(102, 102, 102);
        countLab.numberOfLines =2;
        [self.contentView addSubview:countLab];
        self.discountLab =countLab;
...
//    }];
    ...
    [self.priceLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.right.equalTo(self.mas_right).offset([AdaptationKit getHorizontalSpace:-30]);
...
//    }];
    ...
    [self.discountLab mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.nameLab.mas_bottom).offset([AdaptationKit getVerticalSpace:20]);
...
- (void)configData:(Goods *)model{
    ...
    {
        NSDictionary *dict = model.modelList[i];
        rulestr = [NSString stringWithFormat:@"%@参团%@人%0.2f元    ",rulestr,dict[@"people_num"],[dict[@"percent"]floatValue]* model.price/100];
    }
    _discountLab.text = rulestr;
    ...
    NSString *priceStr =[NSString stringWithFormat:@"¥%.02f",model.deposit];
...
//
//    self.numBuyLab.text=[NSString stringWithFormat:@"已秒%zd件",model.enable_store];
//
